...
Date is updated automatically at regular intervals and is sourced from COMTRADE.
...
china_sme_imports_annual_plot <- ggplot(china_sme_imports, aes(Date,Value)) +
  geom_line(size=0.75, color = "red") +
  geom_point(color = "red", size=2) +
  labs(x="Year", y="Import Value (Billions of Dollars)", title="China Annual Semiconductor sme Imports") +
  scale_y_continuous(labels = label_dollar(scale = 1e-9), limits = c(0, NA), expand = expansion(mult = c(0, 0.2))) +
  scale_x_date(breaks="1 year",labels = date_format("%Y")) +
  expand_limits(y = c(0,NA)) +
  theme_classic() +
...
china_sme_imports_table <- kbl(china_sme_imports_formatted,col.names = c("Date","Semiconductor Imports (Millions of $)")) %>%
  kable_minimal(full_width = F) %>%
  column_spec(1,width_min = "6em") %>%
  scroll_box(width = "100%", height = "500px")
